  ...
  return (
    <div className="space-y-5">
      ...
      <div className="mm-card-glow flex flex-wrap items-end gap-3">
        <div><label className="mm-label">Matéria-Prima</label><select data-testid="mov-mp-select" className="mm-input" value={sel} onChange={(e) => { setSel(e.target.value); setData(null); setParams({ de: "", ate: "", si_qtd: "", si_val: "" }); }}><option value="">Selecione...</option>{mps.map((m) => <option key={m.id} value={m.id}>{m.codigo} - {m.descricao}</option>)}</select></div>
